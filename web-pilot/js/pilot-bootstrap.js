...
(function () {
  ...
  function reportErr(err) {
    if (window.PilotErrors?.report) return window.PilotErrors.report(err);
    console.error(err);
    return (err && err.pilotCode) || "RA-UNK";
  }
  ...
  async function applyClassDataText(text) {
    ...
    try {
      ...
      throw coded("RA-PARSE", "Could not parse class JSON: " + (e.message || e));
    }
    if (!data.classes) throw coded("RA-JSON", "Invalid JSON: no classes");
    state.data = data;
    state.classKey = null;
    state.passwords = {};
    state.studentPoints = {};
  ...
  async function resolveTenantContext() {
    ...
    if (slug && window.TenantRegistry) {
      ...
      if (!row) {
        throw coded("RA-SCHOOL", "Unknown school link (?t=" + slug + "). Check tenants.json.");
      }
      ...
      if (!fileId && !row.folderId && !row.classDataFileId) {
        throw coded(
          "RA-FOLDER",
          "This school link has no Drive folder configured yet. Ask your teacher to contact support."
        );
      }
      ...
      } else if (!fileId && row.classDataFileId) {
        fileId = row.classDataFileId;
      }
    }
  ...
  async function renderDriveConnect(app, options) {
    ...
    try {
      ...
      if (!files.length && ctx.folderId && ctx.apiKey) {
        try {
          files = await window.DrivePilotLoader.listJsonFilesInFolder(ctx.folderId, ctx.apiKey);
        } catch (e) {
          listError = e;
        }
      }
    ...
    const reloadLabel =
      files.length === 1 ? "Load " + files[0].name : "Reload latest file from Google Drive";
    ...
    app.innerHTML =
      ...
      (chooseMode
        ...
        : "") +
      '<div id="uploadMsg" class="mt-2 text-teal-100/90 text-sm"></div>' +
    ...
    if (listError) {
      const code = window.PilotErrors?.classify
        ? window.PilotErrors.classify(listError)
        : "RA-UNK";
      const msg = document.getElementById("uploadMsg");
      if (msg) msg.textContent = "Please report error " + code;
    }
    ...
    if (reloadBtn) {
      reloadBtn.onclick = async function () {
        try {
          ...
          if (!fresh.fileId) throw coded("RA-MISS", "No class data file found in Drive.");
  ...
  window.DrivePilot = {
    ...
    async start() {
      ...
      if (hasDriveContext()) {
        ...
        try {
          ...
          throw coded("RA-MISS", "No class data file found.");
        } catch (err) {
          state.view = "upload";
    ...
    async reloadFromDrive() {
      showLoadingScreen();
      const ctx = await resolveTenantContext();
      if (!ctx.fileId) throw coded("RA-MISS", "No class data file found.");
      await loadDriveFileById(ctx.fileId, ctx.apiKey, ctx.tenantSlug);
    },
    ...
    showBanner() {
      ...
      el.innerHTML =
        "<strong>Web</strong> — Reading Adventures from Google Drive" +
        extra +
        ' <span class="opacity-60">(pilot v3)</span>';
    },
