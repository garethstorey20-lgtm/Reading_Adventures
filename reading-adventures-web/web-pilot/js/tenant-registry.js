...
(function () {
  ...
  function tenantFromUrl() {
    ...
    return (params.get("t") || params.get("tenant") || params.get("school") || "")
      .trim()
      .toLowerCase();
  }
  ...
  async function loadTenantsMap() {
    ...
    try {
      ...
      throw coded("RA-NET", "Could not load tenants.json (network): " + (e.message || e));
    }
    if (!res.ok) {
      const code = window.PilotErrors?.httpCode
        ? window.PilotErrors.httpCode("RA-REG", res.status)
        : "RA-REG-" + res.status;
      throw coded(code, "Could not load tenants.json (HTTP " + res.status + ")");
    }
